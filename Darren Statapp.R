...
data_Fi <- data[, c("finess", "rs","F1_D", "F1_D_bis", "F2_D", "F3_D", "F4_D", "F5_D", 
                    ...
                    "F5_O", "F6_O", "F8_O", "F9_O", "CI_F1_D","CI_F5_D", "CI_F6_D",
...
# Filtrer les valeurs aberrantes étant donné qu'il s'agit de taux
...
# Etude des variables de financement

dataO <- data_Fi[, c("finess", "rs", "TT_recette_exploit_ACO", "TT_recette_exploit_MERRIO" )]
dataD <- data_Fi[, c("finess", "rs", "TT_exploit_MIGFIRD", "TT_exploit_ACD", "TT_exploit_MERRID")]

# On enlève les valeurs manquantes

dataO <- na.omit(dataO[c("finess", "rs", "TT_recette_exploit_ACO", "TT_recette_exploit_MERRIO" )])
dataD <- na.omit(dataD[c("finess", "rs", "TT_exploit_MIGFIRD", "TT_exploit_ACD", "TT_exploit_MERRID")])


# Convertir en variables numériques
dataD <- dataD %>% 
  mutate(TT_exploit_MIGFIRD = as.numeric(TT_exploit_MIGFIRD),
         TT_exploit_ACD = as.numeric(TT_exploit_ACD),
         TT_exploit_MERRID = as.numeric(TT_exploit_MERRID))
dataO <- dataO %>% 
  mutate(TT_recette_exploit_ACO = as.numeric(TT_recette_exploit_ACO),
         TT_recette_exploit_MERRIO = as.numeric(TT_recette_exploit_MERRIO))

# On crée une nouvelle varible qui représente la somme des subventions données aux hopitaux

dataD$TT_subventionD <- dataD$TT_exploit_MIGFIRD + dataD$TT_exploit_ACD + dataD$TT_exploit_MERRID
dataO$TT_subventionO <- dataO$TT_recette_exploit_ACO + dataO$TT_recette_exploit_MERRIO
